...
# temps[999]와 같이 없는 인덱스를 사용 시 에러
...
print(i2) # 1 > 출력은 첫 번째로 찾은 1만 출력함

# 리스트 값 추가
# .append(추가할 값)
# 리스트의 가장 마지막에 값을 추가
# 리스트 원본이 수정 (재할당 필요 없음)
nums = [1, 2, 3, 4, 5]
...
new_nums.append(111)
print(nums) # [1, 2, 3, 4, 5, 999, 111]
print(new_nums) # [1, 2, 3, 4, 5, 999, 111]
# 복사한 메모리 주소에 append를 했기 때문에 원본까지 영향을 받는다.

# 이를 해결하기 위해서 .copy()라는 메서드를 사용
new_nums2 = nums.copy()
new_nums2.append(123)
print(nums) # # [1, 2, 3, 4, 5, 999, 111]
# new_nums2에 nums 원본이 아닌 num.copy()를 할당했기 때문에
# new_nums2를 수정해도 nums 원본에 영향이 가지 않는다.
# 따라서 num 배열에 영향을 미치지 않고 사용 가능하다.

# .insert(위치, 값)
# 리스트에서 원하는 위치에 값을 삽입
# nums = [1, 2, 3, 4, 5, 999, 111]
# 원본 배열에 바로 삽입
...
# list1.remove("망고스틴") # 리스트에 없는 값을 삭제하려고 하면 Error가 남
...
print(list1.pop(2)) # 포도  # 삭제한 인덱스 2번의 값인 포도 출력
...
del list2[::2] # 처음인 0번부터 2칸씩 삭제하겠다는 의미
# 따라서 "빨강", "초록", "남색"이 삭제됨
print(list2) # ['노랑', '파랑', '보라']

# 없는 인덱스로 삭제 불가
# del list2[999]
# print(list2) # IndexError: list assignment index out of range
...
n = [37, 2, 8, 109, 1004, -1, 22]
n.reverse()
print(n) # [22, -1, 1004, 109, 8, 2, 37]

# 리스트 안 값의 갯수 구하기
# .count(찾을 값)

f = ["텀블러", "일회용컵", "일회용컵", "텀블러", "텁블러", "일회용컵"]
