...
pd.set_option("display.max_columns", None)  # 생략하지 않고 다 보여주게 하는 코드
...
# 비스켓두께 숫자들만 담긴 Series에
# 13 이상인지 따져보는 연산을 시킨다면?
# Boolean Series 생성: True 아니면 False만 담김
s_boolean = s >= 13
...
# 위에서 생성된 Boolean Series에서 True값들이 모두 몇개일까요?
# = 최초 CSV 파일에서 '비스킷두께' 컬럼의 값들중에 13 이상인 경우는 몇개인가?

# Bollean Series의 sum() 같은 통계를 낸다면
# True = 1, False = 0 처리
